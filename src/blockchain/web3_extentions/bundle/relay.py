...
class Relay(Module):
	...
	def sign_bundle(
		self,
		bundled_transactions: List[Union[RelayBundleTx, RelayBundleRawTx, RelayBundleDictTx]],
	) -> List[HexBytes]:
	...
	sendRawBundle: Method[Callable[[Any], Any]] = Method(RelayRPC.eth_sendBundle, mungers=[send_raw_bundle_munger])
	...
	def send_bundle_munger(
		...
		self.response = RelayBundleResponse(self.w3, signed_txs, target_block_number)
	...
	def simulate(
		...
		evm_timestamp = (
			block_timestamp if block_timestamp is not None else self.extrapolate_timestamp(block_number, self.w3.eth.block_number)
		)
	...
	def call_bundle_munger(
		self,
		signed_bundled_transactions: List[Union[RelayBundleTx, RelayBundleRawTx]],
		evm_block_number,
		evm_block_state_number,
		evm_timestamp,
		opts: Optional[RelayOpts] = None,
	...
	call_bundle: Method[Callable[[Any], Any]] = Method(json_rpc_method=RelayRPC.eth_callBundle, mungers=[call_bundle_munger])
