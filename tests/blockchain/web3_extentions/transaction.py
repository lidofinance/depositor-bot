...
@pytest.mark.unit
def test_get_priority_fee(w3_unit):
    tu = TransactionUtils(w3_unit)
...
@pytest.mark.unit
def test_protector_no_account(w3_unit, caplog):
...
@pytest.mark.unit
def test_protector_create_tx(w3_unit, set_integration_account, caplog):
    caplog.set_level(logging.INFO)
...
@pytest.mark.unit
def test_estimate_gas(w3_unit, set_account):
    tx = Transaction()

    tx.estimate_gas = Mock(return_value=variables.CONTRACT_GAS_LIMIT * 2)
    gas_amount = w3_unit.transaction._estimate_gas(tx, variables.ACCOUNT.address)
