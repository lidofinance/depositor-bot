...
from transport.msg_providers.onchain_transport import build_deposit_message, build_pause_message, build_unvet_message
...
def get_delegated_pause_message(web3, guardian: str, delegate_pk: str) -> PauseMessage:
    """Build a DSMv5 pause message: the digest binds the guardian contract, the delegate signs it.
    ...
    msg_hash = web3.solidity_keccak(['bytes32', 'address', 'uint256'], [prefix, guardian, latest.number])
    signed = web3.eth.account._sign_hash(msg_hash, private_key=delegate_pk)

    message = build_pause_message(
        block_number=latest.number,
        guardian=guardian,
        r=signed.r.to_bytes(32, 'big'),
        vs=from_hex_string_to_bytes(compute_vs(signed.v, '0x' + signed.s.to_bytes(32, 'big').hex())),
...
def get_delegated_unvet_message(
    web3,
    guardian: str,
    delegate_pk: str,
    module_id: int,
    ...
    """Build a DSMv5 unvet message: the digest binds the guardian contract, the delegate signs it.

    | UNVET_MESSAGE_PREFIX | guardian | blockNumber | blockHash | stakingModuleId | nonce | operatorIds | vettedKeys |
    """
    latest = web3.eth.get_block('latest')
    ...
    msg_hash = web3.solidity_keccak(
        ...
        [prefix, guardian, latest.number, latest.hash, module_id, nonce, operator_ids, vetted_keys_by_operator],
    )
    signed = web3.eth.account._sign_hash(msg_hash, private_key=delegate_pk)

    message = build_unvet_message(
        block_number=latest.number,
        block_hash=latest.hash,
        guardian=guardian,
        staking_module_id=module_id,
